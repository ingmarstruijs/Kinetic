...
class PartnerProposalRepository {
  ...
  /// Watch the accepted proposal (if any) that originated the task with
  /// [taskTitle]. Used for "from {name}" chrome on tiles and the edit sheet.
  Stream<PartnerProposal?> watchAcceptedProposalForTask({
    required String taskTitle,
  }) {
    final normalized = _normalizeTitle(taskTitle);
    return (_db.select(
