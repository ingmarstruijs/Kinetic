...
class KidsPanelState extends State<KidsPanel> {
        ...
        for (final task in group.tasks) {
          if (task.status == ICalTaskStatus.completed) {
            ...
            await widget.todoRepo?.removeKidsAssignment(
              kidsTaskId: task.uid,
              linkTaskId: icalProp(task.description, 'xKineticLinkTaskId'),
            );
          }
        }
      });
  ...
  bool get _canDeleteKidTask =>
      widget.kidsParticipation &&
      (widget.onDeleteKidTask != null || widget.syncConfig != null);

  /// Accept/reject only when this device is the verifier (or none assigned).
  bool _canVerify(ICalTask task) {
    if (!widget.kidsParticipation) return false;
    final verifier = icalProp(task.description, 'xKineticVerifierLinkId');
    if (verifier == null || verifier.isEmpty) return true;
    final me = widget.myLinkId;
...
class _KidGroupCard extends StatelessWidget {
  ...
  final bool Function(ICalTask task)? canVerify;
                    ...
                    if (open.isEmpty)
                      ...
                      for (final task in open)
                        _KidsTaskTile(
                          ...
                          onAccept: canVerify == null || canVerify!(task)
                              ? onAccept
                              : null,
                          onReject: canVerify == null || canVerify!(task)
                              ? onReject
                              : null,
                        ),
                  ],
