...
class _TaskDetailSheetState extends State<TaskDetailSheet> {
  ...
  Future<void> _loadFamilyConnections() async {
    ...
    try {
      ...
      final otherLinkMembers =
          roster?.otherLinkMembers(widget.myLinkId ?? '') ??
  ...
  Future<FamilyMemberStatus?> _pickLinkMember(
    BuildContext context,
    List<FamilyMemberStatus> linkMembers,
  ) {
    final l10n = AppLocalizations.of(context);
    return showDialog<FamilyMemberStatus>(
      context: context,
      builder: (ctx) => SimpleDialog(
        title: Text(l10n.familyMemberPickerTitle),
        children: [
          for (final member in linkMembers)
            SimpleDialogOption(
              ...
              child: ListTile(
                ...
                subtitle: Text(member.statusLabel(l10n)),
              ),
            ),
        ],
      ),
