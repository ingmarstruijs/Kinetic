...
import 'package:link/todo/widgets/kids_panel.dart';
...
void main() {
  ...
  ICalTask task({
    ...
    final desc = [
      ...
      'xKineticXpReward:$xp',
      if (verifierLinkId != null) 'xKineticVerifierLinkId:$verifierLinkId',
    ].join(';');
  ...
  testWidgets('only the designated verifier is offered accept/reject', (
    tester,
  ) async {
    await tester.runAsync(() async {
      await tester.pumpWidget(
            ...
            body: KidsPanel(
              ...
              pullSharedTasks: () async => [
                ...
                task(
                  ...
                  kidId: 'job',
                  status: ICalTaskStatus.inProcess,
                  verifierLinkId: 'adult-b',
                ),
                task(
                  uid: 'anyone',
                  summary: 'Tafel dekken',
                  kidId: 'job',
                  status: ICalTaskStatus.inProcess,
                ),
              ],
              onAcceptKidTask: (_) async {},
              onRejectKidTask: (_) async {},
            ),
          ),
        ),
      ...
      // All three completions are listed, but only the two this link member may
      // verify (own + unassigned) show the buttons.
      expect(find.text('Kamer opruimen'), findsOneWidget);
