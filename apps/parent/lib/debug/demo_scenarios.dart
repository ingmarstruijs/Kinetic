...
const demoScenarioCatalog = <DemoScenarioInfo>[
  ...
  DemoScenarioInfo(
    ...
    titleNl: 'Kinderen',
    subtitleEn: 'Two enrolled kids with XP and chores',
    subtitleNl: 'Twee gekoppelde kinderen met XP en klusjes',
  ),
  DemoScenarioInfo(
...
class DemoScenarioLoader {
  ...
  void _setFamily({required bool partner, required List<EnrolledKid> kids}) {
    DemoSession.instance.apply(
      ...
      kids: kids,
      kidTasks: kids.isEmpty ? const [] : _demoKidTasks(kids),
    );
  }
  ...
  List<EnrolledKid> _demoKids() {
    ...
    return [
      EnrolledKid(id: 'demo-mees', name: 'Mees', enrolledAt: enrolledAt),
      EnrolledKid(id: 'demo-fien', name: 'Fien', enrolledAt: enrolledAt),
    ];
  }

  List<ICalTask> _demoKidTasks(List<EnrolledKid> kids) {
    ...
    ICalTask chore({
      ...
      return ICalTask(
        uid: const Uuid().v4(),
        summary: summary,
        description: 'xKineticTargetKidId:$kidId;xKineticXpReward:$xp',
        status: status,
        createdAt: now,
        updatedAt: now,
      );
  ...
  Future<void> _seedSuggestions(bool dutch) async {
    ...
    await _proposalRepo.createManualProposal(
      myParentId: DemoSession.partnerId,
